...
export const serviceApi = {
  ...
  async queryByCustomerInfo(query: ServiceQuery): Promise<ServiceRecord[]> {
    ...
    if (query.trackingNumber) {
      queryBuilder = queryBuilder.eq('tracking_number', query.trackingNumber);
    } else if (query.firstName && query.lastName && query.phone) {
      // Müşteri bilgilerine göre ara
      const { data: customers, error: customerError } = await supabase
        .from('customers')
        .select('id')
        .eq('first_name', query.firstName)
        .eq('last_name', query.lastName)
