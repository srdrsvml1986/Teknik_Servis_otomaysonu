...
import { Search, Clock, User, Database, X } from 'lucide-react';
...
export const AuditLogs: React.FC = () => {
  ...
  const loadLogs = async () => {
    try {
      ...
      // Her log için kullanıcı bilgisini ekle
      const enrichedLogs = await Promise.all(
        data.map(async (log) => {
          try {
            const user = await getUserById(log.performed_by);
            return {
              ...log,
              performed_by_user: { email: user?.data?.email || 'Bilinmiyor' }
            };
          } catch (error) {
            console.error(`User ${log.performed_by} bulunamadı:`, error);
            return {
              ...log,
              performed_by_user: { email: 'Bilinmiyor' }
            };
          }
        })
      );
